...
(function () {
    function Radar(paper, cx, cy, r, values, opts) {
        ...
        var tabs = new Array();
        tabs[0] = [3,3,2,1,5,4,5,3];
        tabs[1] = [0,0,0,0,0,0,5,5];
        tabs[2] = [5,4,1,2,1,1,2,3];
        tabs[3] = [5,3,3,5,3,5,1,4];
        tabs[4] = [1,1,5,0,0,1,1,3];
        ...
        var path = function (cx, cy, r, startAngle, values, max) {
            ...
            bg = paper.path(pathData.join(",")).attr({
        ...
        var circle = function (cx, cy, r, angle, value, max, title, i) {
            var rad = $r.rad(angle);
            var p = {
            ...
            items[i] = paper.circle(p.x, p.y, opts.pathCircleInnerRadius * 2).attr({ fill: "#F3712F", stroke:"#000000", "stroke-width":"3", title: title + ": " + value });
        }
        ...
        if (opts.drawMesh) {
            ...
            while (i <= l) {
                ...
                paper.circle(cx, cy, opts.meshSize * i).attr({
                    fill: opts.meshFill,
                    stroke: opts.meshStroke,
                    "stroke-width": opts.meshStrokeWidth
                });
        ...
        if (opts.drawLabels) {
            ...
            while (i--) {
                var textObject = label(cx, cy, r + 20, startAngle + angle * i);
                ...
                var fontSizeMin = 8;
                var fontFamily = "'Pontano Sans' , PingFangTC-Regular, Lantinghei SC Extralight , 'Microsoft JhengHei', 'Lucida Grande', '微軟正黑體', Verdana, Arial, Helvetica, sans-serif";
                for (var fontSize = opts.labelFontSize; fontSize >= fontSizeMin; --fontSize) {
                    text.attr({"font-size": fontSize, "font-family": fontFamily });
                    var tempText = "";
        ...
        if (opts.drawPathCircles) {
            var i = values.length;
            while (i--) {
                circle(cx, cy, r, startAngle + angle * i, values[i], opts.max, opts.labels[i], i);
            }
        }
        ...
        $(".tab").find("a").click(function(event) {
            ...
            animation(cx, cy, r, startAngle, tabs[getIndex], opts.max);
        ...
        var animation = function (cx, cy, r, startAngle, values, max) {
            var time = 500;
            var pathData = [];
            var i = 0, l = values.length;
            while (i < l) {
            ...
            var j = values.length;
            while (j--) {
                var rad = $r.rad(startAngle + angle * j);
                var p = {
                    x: cx + r / max * values[j] * Math.cos(rad),
                    y: cy + r / max * values[j] * Math.sin(rad)
                };

                items[j].animate({cx: p.x, cy: p.y}, time);
            }
            ...
            var anim = $r.animation({path: pathData.join(",")}, time, "<>");
            bg.animate(anim);
        }

        $(window).scroll(function(event) {
            if(isScrolled("#section3") && sectionRun[2]){
                sectionRun[2] = false;

                animation(cx, cy, r, startAngle, tabs[0], opts.max);
            }
        });
    };
    //inheritance
